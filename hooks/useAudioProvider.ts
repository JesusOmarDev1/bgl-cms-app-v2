...
import type { Track } from "@/lib/audio/html-audio"
...
export function useAudioProvider({ tracks = [] }: { tracks?: Track[] } = {}) {
  ...
  const setState = useCallback(
    (
      partial:
        Partial<AudioStore> | ((state: AudioStore) => Partial<AudioStore>)
    ) => {
  ...
  useEffect(() => {
    ...
    // Load the current track's audio so it's ready when user clicks play.
    // Without this, audio.src is never set and play() fails silently.
    const initialTrack = useAudioStore.getState().currentTrack
    if (initialTrack && !htmlAudio.getSource()) {
      htmlAudio.load({ url: initialTrack.url, startTime: 0 }).catch(() => {
        // Silent — errors handled by the audio element's error event
      })
    }
