...
function ComboboxItem<T extends object>({
  ...
  return (
    <ListBoxItemPrimitive
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-md py-1 ps-1.5 pe-8 text-sm outline-hidden select-none data-focused:bg-accent data-focused:text-accent-foreground not-data-[variant=destructive]:data-focused:**:text-accent-foreground data-highlighted:bg-accent data-highlighted:text-accent-foreground not-data-[variant=destructive]:data-highlighted:**:text-accent-foreground data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
    ...
    <ComboBoxValuePrimitive<T> className="contents">
      {({ selectedItems, state }) => (
        ...
        >
          <TagListPrimitive
            className="contents"
            items={selectedItems.filter((item) => item != null) as T[]}
            {...props}
          />
        </TagGroupPrimitive>
      )}
