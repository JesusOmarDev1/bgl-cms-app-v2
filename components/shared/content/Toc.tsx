...
import { type TableOfContentsItem as TocItemData } from "@/hooks/useToc"
import {
  buildTocPath,
  getDepthOffset,
  getDepthPadding,
    ...
    >
      <CollapsibleTrigger>
        <Button
          variant="ghost"
          aria-label={
            activeItemTitle && !open ? `${title}: ${activeItemTitle}` : title
          }
      ...
      <CollapsibleContent data-toc-mobile-content="">
        <div>
          <ScrollArea className={tableOfContentsStyles.mobileScrollArea}>
            <TableOfContentsList className={tableOfContentsStyles.mobileList} />
          </ScrollArea>
        </div>
      </CollapsibleContent>
    </header>
...
export type { TableOfContentsItem as TocItem } from "@/hooks/useToc"

export { useScrollSpy } from "@/hooks/useToc"
